...
uint32_t const square_wave_data[2] = {
    (1 << 13),
    (1 << (13 + 16))
};
...
int main(void)
{
	...
	DEVMAP->GPIOs[GPIOC].REGs.CRH &= ~(0b1111 << 20);			// Make low GPIOC output
	DEVMAP->GPIOs[GPIOC].REGs.CRH |= (0b0011 << 20);			// Make high GPIOC output

	DEVMAP->DMAs[DMA1].REGs.CHN[CHN2].CNDTR = 2; // Transfer size
	DEVMAP->DMAs[DMA1].REGs.CHN[CHN2].CMAR	= (uint32_t) square_wave_data; // Defino la memoria del dma, como mi señal cuadrada				 // Memory source address
	DEVMAP->DMAs[DMA1].REGs.CHN[CHN2].CPAR	= (uint32_t) &DEVMAP->GPIOs[GPIOC].REGs.BSRR; // Peripheral destination address
	...
	DEVMAP->TIMs[TIM2].REGs.PSC   = 0;	// fCK_PSC / (PSC[15:0] + 1)
	DEVMAP->TIMs[TIM2].REGs.ARR   = 17999; // aca uso este ARR para que me quede la frecuencia de 2kHz
